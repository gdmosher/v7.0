...
__global__ void simple_vbo_kernel(float4 *pos, unsigned int width, unsigned int height, float time)
{
    ...
    float freq = 16.0f;									// freq was 4.0, 16.0 is interesting when cosf is patched out below
	float w = sinf(u*freq + time);// *cosf(v*freq + time) * 0.5f;
				// (-4 .. 4) + time
    // write output vertex
    pos[y*width+x] = make_float4(u, w, v, 0.1f);		// 1.0f is like a zoom level 0.1f is much closer
