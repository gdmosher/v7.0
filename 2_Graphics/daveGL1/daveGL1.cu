...
////////////////////////////////////////////////////////////////////////////////
// constants
const unsigned int window_width = 512;// 1024;//512;
const unsigned int window_height = 512;// 1024;// 512;
...
///////////////////////////////////////////////////////////////////////////////
__global__ void simple_vbo_kernel(float4 *pos, unsigned int width, unsigned int height, float time, float g_Zoom, float freq, int func)
{
    unsigned int x = blockIdx.x*blockDim.x + threadIdx.x;
    unsigned int y = blockIdx.y*blockDim.y + threadIdx.y;

    // calculate uv coordinates
    float u = x / (float) width;	// mesh_width = 256			// u is 0 to .99 (a percentage of mesh_width)
    float v = y / (float) height;	// mesh_height = 256		// v is 0 to .99
    u = u*2.0f - 1.0f;				// *2 magnifies it, -1.0 centers it
	v = v*2.0f - 1.0f;		// v is now -1 to 1, as y goes from 0 to 256
	if (u > 0.0 && v > 0.0) if (u > .9 || v > .5 ); else return;
    // calculate simple sine wave pattern
//    float freq = 1.0f;									// freq was 4.0, 16.0 is interesting when cosf is patched out below
	float w;
//	if (func == 2)
	if (func == 2 || (func == 3 && y % 2)){
		w = sinf(u*freq + time);// *cosf(v*freq + time) * 0.5f;
//		w = .00f;		// make it a plane
	}
	else{
		w = sinf(u*freq + time) *cosf(v*freq + time) * 0.5f;
		// (-4 .. 4) + time
...
}							//   x,y,z,w				// 10.0f is quite far away
// set break point for x==255 && y == 255 works! or == 0 works too! to single step
// actually any pair of == on x && y, should break the kernel ONCE

void launch_kernel(float4 *pos, unsigned int mesh_width,
                   unsigned int mesh_height, float time)
{
    // execute the kernel
    dim3 block(8, 8, 1);	// 16, 16 works too   4, 4 works too but is too small only 16 lanes
    dim3 grid(mesh_width / block.x, mesh_height / block.y, 1);
    simple_vbo_kernel<<< grid, block>>>(pos, mesh_width, mesh_height, time, g_Zoom, g_freq, g_func);
	printf("g_Zoom = %f\n", g_Zoom);
}
...
//	static GLfloat r = 0.1, g = 0.9, b = 0.0;		// my code to play with colors
	static GLfloat r = 1.0, g = 0.0, b = 0.0, colorStep = .001;		// my code to play with colors
	static int colorPhase = 1;
    glEnableClientState(GL_VERTEX_ARRAY);
//	glColor3f(r += .001, g -= .001, (b += .0001));	// red, green, blue
	glColor3f(r, g, b);	// red, green, blue
	switch (colorPhase) {
	case 1:     // red down, green up
		if (r > 0){ r -= colorStep; g += colorStep; }
		else { colorPhase = 2; }
		break;
	case 2:     // green down, blue up
		if (g > 0){ g -= colorStep; b += colorStep; }
		else { colorPhase = 3; }
		break;
	case 3:     // blue down, red up
		if (b > 0){ b -= colorStep; r += colorStep; }
		else { colorPhase = 1; }
		break;
	}
//	printf("RGB Colors = %f %f %f ColorPhase = %d\n", r, g, b, colorPhase);
    ...
    glutSwapBuffers();								// break at hitcount = 315 to catch origin dot

	g_fAnim += g_fAnimIncrement;// *4;              // was 1.0f
	printf("g_fAnim = %f\n", g_fAnim);
	printf("g_fAnimIncrement = %f\n", g_fAnimIncrement);
	printf("rotate_x = %f\n", rotate_x);
	printf("rotate_y = %f\n", rotate_y);
...
void keyboard(unsigned char key, int /*x*/, int /*y*/)
{
	...
	case ('d') :    //  speed up
		...
		return;
	case ('-') :    //  slow down
		g_fAnimIncrement /= 10.0;
		return;
	case ('+') :    //  speed up
		g_fAnimIncrement *= 10.0;
		return;
	case ('i') :    //  zoom in
		g_Zoom /= 1.1f;// -= 0.1f;// /= 10.0;
		return;
	case ('o') :    //  zoom out
		g_Zoom *= 1.1f;// += 0.1f;// *= 10.0;
		return;
